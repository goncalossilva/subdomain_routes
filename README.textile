...
h2. Working with multiple top-level domains (this feature is only present in this fork)

You can easily work with multiple top-level domains. If users can reach your app on "domain.com" and "awesome.myapp.com" you only need to:

<pre>
SubdomainRoutes::Config.static_domains = ["domain.com", "static.myapp.com"]
</pre>

You should still define <code>SubdomainRoutes::Config.domain_length</code>, since it will be used for all other domains not included in SubdomainRoutes::Config's <code>static_domains</code>.
